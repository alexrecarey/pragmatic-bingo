...
Elm.Bingo.make = function (_elm) {
   ...
   var totalPoints = function (entries) {
      return A3($List.foldl,
      F2(function (e,x) {    return e.points + x;}),
      0,
      A2($List.filter,
      function (_) {
         return _.wasSpoken;
   ...
   var addNewEntry = function (address) {
      return A2($Html.form,
      _U.list([$Html$Attributes.id("newEntry")]),
      _U.list([A2($Html.input,
              _U.list([$Html$Attributes.name("phrase")]),
              _U.list([]))
              ,A2($Html.input,
              _U.list([$Html$Attributes.name("points")]),
              _U.list([]))
              ,A2($Html.button,
              _U.list([A2($Html$Events.onClick,address,A2(Add,"te",2))]),
              _U.list([$Html.text("Add")]))]));
   };
   ...
   var initialModel = {entries: _U.list([A3(newEntry,
                                        "Doing Agile the hard way",
                                        200,
                                        2)
                                        ,A3(newEntry,"Future Proof",100,1)
                                        ,A3(newEntry,"Cloud",300,3)
                                        ,A3(newEntry,"Iterate",150,4)
                                        ,A3(newEntry,"Reactive",250,5)])
                      ,phraseInput: ""
                      ,pointsInput: ""
                      ,nextID: 5};
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return model;
         case "Sort": return _U.update(model,
           ...
           model.entries)});
         case "Delete":
         var remainingEntries = $List.filter(function (e) {
              return !_U.eq(e.id,_p0._0);
           });
           return _U.update(model,
           {entries: remainingEntries(model.entries)});
         case "Mark": var updateEntries = function (e) {
              return _U.eq(e.id,_p0._0) ? _U.update(e,
              {wasSpoken: $Basics.not(e.wasSpoken)}) : e;
           };
           return _U.update(model,
           {entries: A2($List.map,updateEntries,model.entries)});
         case "UpdatePhraseInput": return _U.update(model,
           {phraseInput: _p0._0});
         case "UpdatePointsInput": return _U.update(model,
           {pointsInput: _p0._0});
         default: var maxID = A2($Maybe.withDefault,
           1,
           ...
           model.entries)));
           var e = A3(newEntry,_p0._0,_p0._1,maxID + 1);
           return _U.update(model,
           {entries: A2($List._op["::"],e,model.entries)});}
   });
